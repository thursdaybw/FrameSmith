...
export function buildAccessUnitPlanFragmentFromTrack({ track }) {
    ...
    const access_units = iterateTrackClipItemsWithProvenance({
        track,
        iterateClipItems: (clip) => {
            ...
            for (const accessUnit of clip.iterateAccessUnits()) {
                access_units.push({
                    pts: accessUnit.pts,
                    dts: accessUnit.dts,
    ...
    return createAccessUnitPlanFragment({ access_units });
}
 ...
 */
function iterateTrackClipItemsWithProvenance({ track, iterateClipItems }) {
    if (!track) {
    ...
    for (const clip of track.clips) {
        for (const item of iterateClipItems(clip)) {
            ...
            // Preserve authored object identity.
            // Attach provenance by mutation, not cloning.
            item.clip = clip;
            item.track = track;
