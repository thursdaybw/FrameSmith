...
/**
 * End-to-end NativeMuxer conformance against ffmpeg.
 *
 * This test is a HARD GATE.
 * Any failure here indicates a real regression.
 */
export async function testNativeMuxer_EndToEnd_Conformance_ffmpeg() {
    ...
    // ---------------------------------------------------------
    const mdat = emitMdatBox({ payload: mdatPayload });

    const parts = layout.fileBoxOrder.map(type => {
        if (type === "ftyp") return serializeBoxTree(ftyp);
        if (type === "free") return serializeBoxTree(emitFreeBox());
        if (type === "mdat") return serializeBoxTree(mdat);
        if (type === "moov") return serializeBoxTree(committedMoov);
        throw new Error(`Unknown box ${type}`);
    });

    const outBytes = new Uint8Array(parts.reduce((n, b) => n + b.length, 0));
    let offset = 0;
    for (const p of parts) {
        outBytes.set(p, offset);
        offset += p.length;
    }
