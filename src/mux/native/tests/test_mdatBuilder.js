...
export async function testMdatBuilder() {

    console.log("=== testMdatBuilder ===");

    // ---- TEST 1: no samples ---------------------------------------
    let mdat = buildMdatBox([]);
    ...
    // ---- TEST 4: input mutation does NOT affect build --------------
    const original = new Uint8Array([5, 6, 7]);
    const sOrig = { data: original };

    const mdatBefore = buildMdatBox([sOrig]);
    original[0] = 99;

    if (mdatBefore[8] !== 5) {
        throw new Error("FAIL: buildMdatBox must not read mutated input data");
    }

    console.log("PASS: mdatBuilder tests");
}
