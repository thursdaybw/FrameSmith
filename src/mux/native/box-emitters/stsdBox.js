import { emitAvc1Box } from "./stsdBox/avc1Box.js";
 ...
 * - Fails early
 */
export function emitStsdBox(params) {
    if (!params || typeof params !== "object") {
        throw new Error("emitStsdBox: parameter object is required");
    }
    ...
    for (const key of Object.keys(params)) {
        if (!allowedKeys.includes(key)) {
            throw new Error(
                `emitStsdBox: unknown parameter '${key}'`
            );
        }
    }
    ...
    for (const key of requiredKeys) {
        if (!(key in params)) {
            throw new Error(
                `emitStsdBox: missing required parameter '${key}'`
            );
        }
    }
    ...
    if (!Number.isInteger(width) || width <= 0) {
        throw new Error(
            "emitStsdBox: width must be a positive integer"
        );
    }

    if (!Number.isInteger(height) || height <= 0) {
        throw new Error(
            "emitStsdBox: height must be a positive integer"
        );
    }

    if (codec !== "avc1") {
        throw new Error(
            "emitStsdBox: only 'avc1' codec is supported"
        );
    }

    if (!(avcC instanceof Uint8Array) || avcC.length === 0) {
        throw new Error(
            "emitStsdBox: avcC must be a non-empty Uint8Array"
        );
    }

    if (typeof compressorName !== "string") {
        throw new Error(
            "emitStsdBox: compressorName must be a string"
        );
    }
