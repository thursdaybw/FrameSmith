...
import { validateTextOverlay } from "./textOverlayValidator.js"; // optional but recommended
...
export function drawTextOverlayForTime(t, ctx, canvas, captions, presetName = "default") {
    const seg = captions.find(c => t >= c.start && t < c.end);
    if (!seg) return;

    // Optional dev safety check
    if (typeof validateTextOverlay === "function") {
        validateTextOverlay(seg);
    }
